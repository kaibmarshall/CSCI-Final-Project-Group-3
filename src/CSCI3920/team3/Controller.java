...
public class Controller implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        inventoryList.getItems().addAll(Item);

    }

    public void search(ActionEvent event) {
        inventoryList.getItems().clear();
        inventoryList.getItems().addAll(searchList(searchBar.getText(), Item));
    }
...
/*
    ...
    TableView.getItems().addAll(searchList(searchBar.getText(),words));

    private List<String> searchWordsArray = Arrays.asList(searchwords.trim().split(" "));
            return listOfStrings.stream().filter(input -> {
                return searchWordsArray.stream().allMatch(word -> {
                    input.toLowerCase().contains(word.toLowerCase()));
                }).collect(Collectors.toList());
